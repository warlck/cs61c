...
public class DocWordCount {
    ...
    public static class DocWordCountMap extends Mapper<Text, Text, Text, LongWritable> {
        ...
        public void map(Text key, Text value, Context context)
                throws IOException, InterruptedException {  //context is name to object bindings


            /* MODIFY THE BELOW CODE */
            Matcher matcher = WORD_PATTERN.matcher(value.toString());
            HashSet words = new HashSet();  //all words we have
            while (matcher.find()) {  //Attempts to find the next subsequence of the input sequence that matches the pattern.
                String current = matcher.group();  //current word we are looking at that matched previous match 
                word.set(current); //current is the string subs. that matches, so we are setting in word object the content of current
                if (!(words.contains(word))) { //while the specified element is not in the HashSet
                    context.write(word, ONE); //write it, to generate a key value pair
                    words.add(word);  //we add it to the HashSet
                }
            } 
